...
export async function fomoCandidatesLoop(): Promise<void> {
  ...
  for (const [{ chain, address: pairAddr }, info] of activeWatch.entries()) {
    ...
    const chainCfg  = CHAINS.find(c => c.id === chain || c.gecko === chain);
    ...
    if (!flow.hasData && ageMs > FOMO_NO_WS_DROP_MS) {
      ...
      dropWatchCandidate(pairAddr, chain, `no WS confirmation after ${Math.round(ageMs / 60_000)}m`);
      continue;
    }
    ...
    if (
      ...
      dropWatchCandidate(pairAddr, chain, `selling confirmation netVol:${netVolF.toFixed(3)}`);
      continue;
    }
    ...
    if (!survivedPump) {
      ...
      dropWatchCandidate(pairAddr, chain, `dumped after block pricePct:${pricePct}%`);
      continue;
    }
    ...
    promoteHotCandidate(pairAddr, chain, "FOMO");
    activeWatch.delete(chain, pairAddr);

    const chainCfgF = CHAINS.find(c => c.id === chain || c.gecko === chain);
